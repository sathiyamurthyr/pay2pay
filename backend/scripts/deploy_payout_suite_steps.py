...
STATEMENTS = [
    ...
    # 4. reverse_failed_payout_transaction (Define before update_payout_transaction_status)
    """
    ...
    # 5. update_payout_transaction_status
    """
    ...
    BEGIN
        ...
        IF (v_current_month_used + p_amount) > v_monthly_limit THEN
            RETURN QUERY SELECT FALSE, 'FAILED'::VARCHAR(30), NULL::VARCHAR(100), p_amount, 0.00::NUMERIC(18,2), 0.00::NUMERIC(18,2), 0.00::NUMERIC(18,2), 'MONTHLY_LIMIT_EXCEEDED'::VARCHAR(50), 
                FORMAT('Customer monthly limit exceeded. Limit: Rs.%s, Used: Rs.%s, Requested: Rs.%s', v_monthly_limit, v_current_month_used, p_amount)::VARCHAR(500);
            RETURN;
        END IF;
...
async def deploy():
    ...
    print("\nAll 6 core Payout database functions deployed successfully!")
