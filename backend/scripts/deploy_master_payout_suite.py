...
MASTER_SQL = """
-- ============================================================
-- PAY2PAY - COMPLETE AUTHORITATIVE PAYOUT DATABASE SUITE
-- SUPABASE / POSTGRESQL (LIVE SCHEMA ALIGNED)
-- ============================================================

CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- ============================================================
-- 1. SEQUENCE FOR 5-DIGIT UNIQUE TXN ID SUFFIX
-- ============================================================
...
-- ============================================================
-- 2. AUTHORITATIVE PAYOUT TXN ID GENERATOR
-- Format: <VENDOR_FIRST_CHAR>PAY<DDMMYYHHMM><5-DIGIT-UNIQUE>
...
BEGIN
    ...
    LOOP
        ...
        -- Protect against duplicate under sequence recycling
        IF NOT EXISTS (
            SELECT 1
...
-- ============================================================
-- 3. WALLET BALANCE UPDATE (DYNAMIC LINES & SEQUENTIAL BALANCES)
--
-- Rules:
-- 1. Changes wallet_balance ONCE by p_total_amount.
-- 2. Inserts ONLY component ledger lines (Amount, Charge, GST).
-- 3. NEVER inserts a total "Wallet Debit" line.
-- 4. Computes sequential running opening & closing balances for each line.
-- 5. Full support for live schema columns:
--    tenant_id, company_id, retailer_id, retailer_name,
...
--    balance_before, balance_after, status, narration, partition keys, date keys
-- ============================================================
...
BEGIN
    ...
    -- 2. Duplicate Check per TxnID & EntryType
    IF EXISTS (
        SELECT 1
        FROM public.transactions t
    ...
    -- 3. Lock Retailer Wallet FOR UPDATE
    SELECT
    ...
    IF v_wallet_id IS NULL THEN
        RETURN QUERY SELECT FALSE, p_txn_id, 0.00::NUMERIC(18,2), 0.00::NUMERIC(18,2), p_total_amount, 'FAILED'::VARCHAR(30), 'WALLET_NOT_FOUND'::VARCHAR(100), 'Active retailer wallet not found'::VARCHAR(500);
        RETURN;
    END IF;
    ...
    -- 7. Insert ONLY Component Lines with Sequential Running Balances
    ...
    IF p_payout_amount > 0 THEN
        ...
        INSERT INTO public.transactions (
            ...
            day_key, week_key, month_key, quarter_key, year_key, date_key, time_key,
        ...
        VALUES (
            ...
            p_vendor_id, p_vendor_name, p_txn_id, p_ref_id, p_table_ref_id,
            ...
            v_day_key, v_week_key, v_month_key, v_quarter_key, v_year_key, v_date_key, v_time_key,
            v_partition_year, v_partition_month, v_partition_day,
            TRUE, FALSE, NOW(), p_created_by
        );

        v_running_balance := v_line_after;
    END IF;

    -- Line 2: Payout Charge
    ...
    -- Safety: Verify running balance matches wallet
...
-- Overload for JSON array dynamic lines calling convention
CREATE OR REPLACE FUNCTION public.wallet_balance_update(
    p_tenant_id        UUID,
    p_company_id       UUID,
    p_retailer_id      UUID,
...
DECLARE
    ...
    v_time_key           INTEGER;
BEGIN
    IF p_tenant_id IS NULL OR p_company_id IS NULL OR p_retailer_id IS NULL OR p_txn_id IS NULL THEN
        RAISE EXCEPTION 'Context parameters are required';
    END IF;
    ...
    LOOP
        ...
        VALUES (
            gen_random_uuid(), p_tenant_id, p_company_id, p_retailer_id, v_line->>'retailer_name',
            ...
            p_service_name, p_wallet_type, v_line->>'user_type',
...
-- ============================================================
-- 4. FAILED PAYOUT REVERSAL (SAME TXN ID, CREDIT COMPONENTS)
-- ============================================================
...
BEGIN
    ...
    -- 2. Read Original Payout Context & Lines
    SELECT
        MAX(t.tenant_id),
        MAX(t.company_id),
        MAX(t.retailer_id),
    ...
    -- 3. Execute CREDIT via wallet_balance_update using SAME TxnID
...
async def deploy():
    print("Deploying updated Pay2Pay Master Payout Suite...")
    async with AsyncSessionLocal() as session:
        await session.execute(text(MASTER_SQL))
        await session.commit()
        print("Pay2Pay Master Payout Suite deployed successfully!")
