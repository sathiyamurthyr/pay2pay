"""
Enterprise Full Database Truncate & Clean Reset Script for Customer & Beneficiary tables.
Uses TRUNCATE TABLE ... CASCADE to remove all lingering test rows.
"""
...
async def truncate_and_reset(seed_default: bool = True):
    async with AsyncSessionLocal() as db:
        print("=========================================================")
        print("TRUNCATING ALL CUSTOMER & BENEFICIARY TABLES (CASCADE)...")
        print("=========================================================")

        # Execute CASCADE TRUNCATE on all customer and beneficiary tables
        truncate_sql = text("""
            TRUNCATE TABLE 
        ...
        await db.commit()
        print("--> All customer & beneficiary tables TRUNCATED successfully (0 rows remain).")
        ...
        print("=========================================================")
        print("RE-SEEDING SINGLE TEST CUSTOMER & BENEFICIARY COMPLETE!")
        print("=========================================================")
        print(f"Customer: {customer.full_name} ({customer.customer_number}) | Mobile: {customer.mobile_number}")
        print(f"Beneficiary: {beneficiary.full_name} ({beneficiary.beneficiary_number}) | Account: {bank_account.account_number}")
        print("=========================================================")


if __name__ == "__main__":
    import sys
    clean_only = "--empty" in sys.argv
    asyncio.run(truncate_and_reset(seed_default=not clean_only))
