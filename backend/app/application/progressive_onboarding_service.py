...
class ProgressiveOnboardingService:
    ...
    async def verify_gst(db: AsyncSession, registration_id: str, gst_number: str) -> Dict[str, Any]:
        ...
        # Validate PAN ↔ GST relationship:
        # Standard GSTIN format is 2 digits state code + 10 char PAN + 1 entity num + 1 'Z' + 1 check digit
        gst_pan_part = clean_gst[2:12]
        draft_pan = draft.draft_data.get("pan_number") or draft.draft_data.get("pan", {}).get("pan_number")
        
        if draft_pan and gst_pan_part != draft_pan.upper():
            return {
                "status": "ERROR",
                "message": f"The GST details could not be verified against the provided business/PAN information ({draft_pan})."
            }
    ...
    async def submit_registration(db: AsyncSession, registration_id: str) -> Dict[str, Any]:
        """Final Submit: Revalidate all mandatory onboarding requirements, lock draft status to KYC_SUBMITTED and generate Application Ref."""
        d_stmt = select(RegistrationDraftModel).where(RegistrationDraftModel.registration_id == registration_id)
        draft = (await db.execute(d_stmt)).scalars().first()
        if not draft:
            return {"status": "ERROR", "message": "Invalid registration ID."}
        ...
        comp_steps = set(draft.completed_steps or [])

        # 1. Mobile verification check
        if not draft.mobile_number or (1 not in comp_steps and 2 not in comp_steps):
            return {"status": "ERROR", "message": "Mobile verification is mandatory before completing registration."}

        # 2. Email verification check
        if not draft.email or 4 not in comp_steps:
            return {"status": "ERROR", "message": "Email verification is mandatory before completing registration."}

        # 3. PAN check
        if 6 not in comp_steps and not draft_d.get("pan"):
            return {"status": "ERROR", "message": "PAN verification is required before completing registration."}

        # 4. Conditional GST check (if business entity)
        if draft.is_business:
            if 66 not in comp_steps and not draft_d.get("gst"):
                return {"status": "ERROR", "message": "GST verification is required for business entities before completing registration."}

        # 5. Aadhaar check
        if 7 not in comp_steps and not draft_d.get("aadhaar"):
            return {"status": "ERROR", "message": "Aadhaar eKYC verification is required before completing registration."}

        # 6. Bank account check
        if 8 not in comp_steps and not draft_d.get("bank"):
            return {"status": "ERROR", "message": "Bank account verification is required before completing registration."}
