            ...
            if v_code in ("UTKAL", "UTKAL_DIGITAL"):
                try:
                    ...
                    if res_status in ("SUCCESS", "PENDING") or payout_res.get("success"):
                        ...
                        tx.utr = payout_res.get("utr") or f"UTK{txn_ref}"
                        tx.vendor_order_id = str(payout_res.get("order_id") or payout_res.get("vendor_tx_id") or "")
                        tx.response_message = payout_res.get("message", "Utkal Payout executed successfully")
                    else:
                        tx.status = "FAILED"
                        tx.response_message = payout_res.get("message", "Utkal Gateway execution rejected")
