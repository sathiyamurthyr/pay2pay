...
def build_unified_transactions_query(
    ...
    WITH unified_txns AS (
        ...
        SELECT 
            ...
            COALESCE(e.wallet_before, 0.0)::float AS previous_balance,
            0.0 AS cr,
            COALESCE(e.net_debit, (e.amount + COALESCE(e.charges, 0.0) + COALESCE(e.gst_amount, 0.0)))::float AS dr,
            COALESCE(e.wallet_after, GREATEST(0.0, COALESCE(e.wallet_before, 0.0) - COALESCE(e.net_debit, (e.amount + COALESCE(e.charges, 0.0) + COALESCE(e.gst_amount, 0.0)))))::float AS current_balance,
        ...
        SELECT 
            ...
            COALESCE(e_rev.wallet_after, GREATEST(0.0, COALESCE(e_rev.wallet_before, 0.0) - COALESCE(e_rev.net_debit, 0.0)))::float AS previous_balance,
            COALESCE(e_rev.net_debit, (e_rev.amount + COALESCE(e_rev.charges, 0.0) + COALESCE(e_rev.gst_amount, 0.0)))::float AS cr,
            0.0 AS dr,
            COALESCE(e_rev.wallet_before, 0.0)::float AS current_balance,
        ...
        SELECT 
            ...
            COALESCE(p.wallet_before, 0.0)::float AS previous_balance,
            0.0 AS cr,
            COALESCE(p.net_debit, (p.amount + COALESCE(p.charges, 0.0) + round((COALESCE(p.charges, 0.0) * 0.18)::numeric, 2)))::float AS dr,
            GREATEST(0.0, COALESCE(p.wallet_before, 0.0) - COALESCE(p.net_debit, (p.amount + COALESCE(p.charges, 0.0) + round((COALESCE(p.charges, 0.0) * 0.18)::numeric, 2))))::float AS current_balance,
