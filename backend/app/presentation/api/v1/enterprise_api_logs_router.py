...
async def list_api_logs(
    ...
    # 2. Server-Side Comprehensive Search
        ...
        # Only add slow TEXT ILIKE scans (error_message, failure_reason) for short keyword
        # searches (< 10 chars), NOT for transaction ID lookups — those would force a
        # full-table scan across 100k+ rows and take 9+ seconds.
        _is_id_like = (
            len(search_term) >= 10 and (
                search_term[:4].upper() in ("UPAY", "TXN-", "UTR-", "LOG-", "REF-") or
                search_term.isdigit()
            )
        )
        if not _is_id_like and len(search_term) < 40:
            search_clauses.extend([
