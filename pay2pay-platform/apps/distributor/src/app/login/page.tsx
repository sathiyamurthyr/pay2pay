...
export default function DistributorLoginPage() {
  ...
  return (
    <main
      className={`w-full min-h-screen min-h-[100dvh] font-sans antialiased flex flex-col lg:flex-row 2xl:max-w-[2560px] 2xl:mx-auto transition-colors duration-300 selection:bg-amber-500/30 selection:text-amber-200 ${
        darkMode ? "bg-slate-950 text-slate-50" : "bg-slate-900 text-slate-100"
      }`}
    >
      ...
      <div className="hidden lg:flex lg:w-[58%] xl:w-[60%] 2xl:w-[62%] min-h-screen sticky top-0 h-screen overflow-hidden flex-col justify-between shrink-0 border-r border-slate-800/60 dark:border-slate-800/80">
        <HeroSection darkMode={darkMode} portalRole="DISTRIBUTOR" />
      </div>
      ...
      <div className="lg:hidden w-full p-4 sm:p-6 bg-slate-950/80 backdrop-blur-xl border-b border-white/[0.08] flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-amber-500 via-yellow-400 to-amber-600 p-0.5 shadow-md shadow-amber-500/20 flex items-center justify-center shrink-0">
            <div className="w-full h-full bg-slate-950 rounded-[10px] flex items-center justify-center">
              <span className="text-xs font-black bg-gradient-to-r from-amber-400 via-yellow-300 to-amber-500 bg-clip-text text-transparent">
                P2P
              </span>
            </div>
          </div>
          <div>
            <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] font-bold">
              ...
              <span>Distributor Workstation</span>
            </div>
        ...
        <button
          ...
          className="p-2 rounded-xl bg-slate-900/80 border border-slate-800 text-slate-400 hover:text-amber-400 transition-colors flex items-center justify-center"
          title="Toggle Theme"
      ...
      <div className="w-full lg:w-[42%] xl:w-[40%] 2xl:w-[38%] min-h-screen flex flex-col justify-center items-center overflow-y-auto shrink-0 p-3 sm:p-6 lg:p-8">
        <div className="w-full max-w-md my-auto">
          <AuthPanel portalRole="DISTRIBUTOR" darkMode={darkMode} setDarkMode={setDarkMode} />
        </div>
