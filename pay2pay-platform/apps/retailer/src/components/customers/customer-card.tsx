...
export interface CustomerCardProps {
  ...
  onVerifyAadhaar?: (customer: CustomerCardData) => void;
    ...
    >
      ...
      <div className="space-y-4 flex-1 flex flex-col justify-between">
        <div className="space-y-3.5">
          ...
          {/* Metadata Row: eKYC Status Badge + Aadhaar Badge + Risk Badge */}
          <div className="flex items-center justify-between gap-2 text-[12px] pt-1 font-bold flex-wrap">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {/* Dynamic Aadhaar status — driven by backend */}
              {customer.aadhaarStatus === "VERIFIED" ? (
                <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold bg-teal-950/80 text-teal-400 border border-teal-800 whitespace-nowrap shrink-0">
                  <Fingerprint className="w-3 h-3 shrink-0" />
                  <span>Aadhaar ✓</span>
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold bg-orange-950/80 text-orange-400 border border-orange-800 whitespace-nowrap shrink-0">
                  <Fingerprint className="w-3 h-3 shrink-0" />
                  <span>Aadhaar ?</span>
                </span>
              )}
      ...
      <div className="mt-auto pt-3.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2 flex-wrap">
        {/* Verify Aadhaar button — only shown when not verified */}
        {customer.aadhaarStatus !== "VERIFIED" && onVerifyAadhaar && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onVerifyAadhaar(customer);
            }}
            aria-label={`Verify Aadhaar for ${customer.name}`}
            className="h-10 min-h-[40px] px-3 rounded-xl border border-orange-600/70 bg-orange-950/60 text-orange-400 hover:bg-orange-900/60 font-bold text-[11px] flex items-center gap-1.5 transition-all active:scale-95 focus:ring-2 focus:ring-orange-600 focus:outline-none whitespace-nowrap shrink-0"
          >
