...
export const SessionLockScreenOverlay: React.FC = () => {
  ...
  // Dynamic Company & SD/User Metadata from authenticated session
  const companyName = walletData?.company_name || "Pay2Pay Enterprise";
  const userName = walletData?.owner_name || walletData?.retailer_name || "Super Distributor";
  const userCode = walletData?.retailer_code || (walletData as any)?.user_code || "SD-401";
  ...
  const handleGoToLogin = () => {
    try {
      const rawRole = localStorage.getItem("p2p_user_role") || "SUPER_DISTRIBUTOR";
