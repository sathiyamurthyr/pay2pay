...
async def test_otp_send_and_verify():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        send_res = await ac.post("/api/v1/auth/enterprise/login-otp/send", json={"mobile_number": "7013914767", "channel": "WHATSAPP"})
        assert send_res.status_code == 200
        s_data = send_res.json()
        assert s_data["status"] == "SUCCESS"

        verify_res = await ac.post("/api/v1/auth/enterprise/login-otp/verify", json={"mobile_number": "7013914767", "otp_code": "778899"})
