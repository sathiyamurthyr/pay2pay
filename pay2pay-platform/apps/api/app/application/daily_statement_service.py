...
class BankStatementCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count: int):
        self.saveState()
        # Top banner line (Pay2Pay primary cyan/blue)
        self.setStrokeColor(colors.HexColor("#0284c7"))
        self.setLineWidth(3)
        self.line(32, 815, 563, 815)
...
class DailyAccountStatementService:
    ...
    def build_bank_statement_pdf(cls, data: Dict[str, Any]) -> bytes:
        """
        Builds a high-precision bank-style statement PDF with PAN password protection.
        """
        buffer = io.BytesIO()
        pan = (data.get("pan_number") or data.get("retailer_code") or "PAY2PAY").strip().upper()
        ...
        header_data = [
            ...
            [
                ...
                Paragraph(
                    f"Ref: <b>{data.get('statement_number', 'STMT-001')}</b>",
                    ParagraphStyle("Ref", fontName="Helvetica", fontSize=8, alignment=2, textColor=colors.HexColor("#64748B"))
                )
            ]
        ]
        ...
        holder_name = data.get("company_name" if is_admin else "retailer_name", "Valued Client")
        ...
        # 3. Balance Summary - Prominent Closing Balance
        ...
        summary_data = [
            ...
            [
                Paragraph(f"₹{op_bal:,.2f}", card_amt),
                Paragraph(f"+ ₹{cr_tot:,.2f}", card_amt_cr),
                Paragraph(f"- ₹{dr_tot:,.2f}", card_amt_dr),
                Paragraph(f"₹{cl_bal:,.2f}", card_amt_cl)
            ]
        ]
        ...
        for tx in tx_rows:
            t_time = tx.get("txn_time", "")
            t_id = tx.get("txn_id", "")
            t_desc = tx.get("description", "") or tx.get("service", "")
            t_ref = tx.get("reference", "") or "—"
            c_val = float(tx.get("credit", 0.0) or 0.0)
            d_val = float(tx.get("debit", 0.0) or 0.0)
            ...
            table_content.append([
                Paragraph(t_time, tbl_cell),
                Paragraph(t_id, tbl_cell),
                Paragraph(t_desc[:32], tbl_cell),
                Paragraph(t_ref[:20], tbl_cell),
                Paragraph(c_str, tbl_cell_cr if c_val > 0 else tbl_cell_right),
                Paragraph(d_str, tbl_cell_dr if d_val > 0 else tbl_cell_right),
                Paragraph(b_str, tbl_cell_right)
            ])
        ...
        if not tx_rows:
            table_content.append([
                ...
                Paragraph("—", tbl_cell_right),
                Paragraph(f"₹{cl_bal:,.2f}", tbl_cell_right)
            ])
        ...
        table_content.append([
            ...
            Paragraph("", tbl_cell),
            Paragraph("", tbl_cell),
            Paragraph(f"<b>₹{cr_tot:,.2f}</b>", tbl_cell_cr),
            Paragraph(f"<b>₹{dr_tot:,.2f}</b>", tbl_cell_dr),
            Paragraph(f"<b>₹{cl_bal:,.2f}</b>", tbl_cell_right)
        ])

        tx_table = Table(table_content, colWidths=[75, 75, 115, 85, 60, 60, 61], repeatRows=1)
        t_style = [
    ...
    async def generate_retailer_statement(
        ...
        async def _run(db: AsyncSession) -> Dict[str, Any]:
            ...
            company_id = d.get("company_id") or GlobalContext.company_id or "b1e02b56-7742-4549-a4ef-31f5378f00a8"
            ...
            if not reconciled:
                ...
                error_message = (
                    f"Reconciliation mismatch! Diff: ₹{d.get('reconciliation_difference', 0.00)}. "
                    f"Opening ₹{d.get('opening_balance')} + Cr ₹{d.get('total_credit')} - Dr ₹{d.get('total_debit')} "
                    f"!= Cl ₹{d.get('closing_balance')}."
                )
                ...
                if recipient_email and "@" in recipient_email:
                    try:
                        ...
                        email_status = "SENT" if email_res.get("delivered") else "FAILED"
                        if not email_res.get("delivered"):
            ...
            rec_res = await db.execute(
                ...
                """),
                {
                    ...
                    "p_metadata_json": json.dumps({"retailer_code": retailer_code, "pan_set": bool(d.get("pan_number"))})
                }
            )
    ...
    async def generate_admin_statement(
        ...
        async def _run(db: AsyncSession) -> Dict[str, Any]:
            ...
            tenant_id = d.get("tenant_id") or "00000000-0000-0000-0000-000000000001"
            stmt_date_str = statement_date.strftime("%Y-%m-%d")
            stmt_num = f"STMT-ADMIN-CMP-{statement_date.strftime('%Y%m%d')}"
            d["statement_number"] = stmt_num
            ...
            admin_email = getattr(settings, "ADMIN_STATEMENT_EMAIL", "admin@pay2pay.in")

            if not reconciled:
                email_status = "SUPPRESSED_RECONCILIATION_FAILED"
                error_message = (
                    f"Admin reconciliation mismatch! Diff: ₹{d.get('reconciliation_difference', 0.00)}. "
                    f"Platform Opening ₹{d.get('opening_balance')} + Cr ₹{d.get('total_credit')} - Dr ₹{d.get('total_debit')} "
                    f"!= Cl ₹{d.get('closing_balance')}."
                )
    ...
    async def execute_daily_statement_job(
        ...
        target_company_id = company_id or GlobalContext.company_id or "b1e02b56-7742-4549-a4ef-31f5378f00a8"

        logger.info(f"Starting daily automatic account statement batch for {target_date} (Company: {target_company_id})")
        ...
        # 3. Generate Company/Admin Statement
        try:
            ...
            logger.error(f"Failed to generate Admin statement for company {target_company_id}: {ex}")
            stats["errors"].append({"target": "ADMIN", "error": str(ex)})
    ...
    async def resend_statement_email(
        ...
        async def _run(db: AsyncSession) -> Dict[str, Any]:
            ...
            recipient_name = d.get("retailer_name") or d.get("retailer_code") or "Account Holder"
