...
export default function DistributorApprovalsPage() {
  ...
  const fetchPending = async () => {
    try {
      ...
      const dists = res.data?.data?.distributors || [];
      setItems(dists);
    } catch (err: any) {
      console.error("Error fetching pending distributors:", err);
      setError(err?.response?.data?.detail || err?.message || "Failed to load pending distributor approvals.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleAction = async (distId: string, action: "APPROVE" | "REJECT") => {
    try {
      setProcessingId(distId);
      setActionSuccess(null);
      await apiClient.post(`/admin/retailer-control/distributor-approvals/${distId}`, {
        action,
        reason: `Administrative ${action.toLowerCase()} action`,
      });

      setActionSuccess(`Distributor successfully ${action === "APPROVE" ? "approved and activated" : "rejected"}.`);
      fetchPending();
    } catch (err: any) {
      console.error(`Error performing ${action}:`, err);
      alert(err?.response?.data?.detail || err?.message || `Failed to ${action} distributor.`);
    } finally {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-white/[0.06]">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400">
            Review and approve distributor partner registrations. Approving auto-maps the 4-tier hierarchy.
          </p>
        </div>
      ...
      <div className="rounded-2xl bg-gradient-to-b from-white/[0.04] to-white/[0.01] border border-white/[0.08] overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="py-16 text-center space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-300">All Distributor Approvals Clear!</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              There are no distributor partners currently pending administrative KYC or approval.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-white/[0.08] bg-white/[0.02] text-slate-400 font-semibold uppercase tracking-wider text-[10px]">
                  ...
                  <th className="p-4">Super Distributor Ref</th>
              ...
              <tbody className="divide-y divide-white/[0.04]">
                {items.map((dist) => (
                  <tr key={dist.distributor_id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="p-4">
                      <div>
                    ...
                    <td className="p-4 text-slate-300">
                      <div>{dist.owner_name}</div>
                      <div className="text-[10px] text-slate-500">{dist.mobile} • {dist.email}</div>
                    </td>
                    ...
                    <td className="p-4 font-mono text-amber-300">
                      SD #{dist.super_distributor_ref_id || "1 (Default)"}
                    </td>

                    <td className="p-4 text-slate-400 text-[11px]">
                      {dist.created_at ? new Date(dist.created_at).toLocaleDateString("en-IN") : "—"}
                    </td>

                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => handleAction(dist.distributor_id, "APPROVE")}
                          disabled={processingId === dist.distributor_id}
                        ...
                        >
                          {processingId === dist.distributor_id ? (
                        ...
                        <button
                          onClick={() => handleAction(dist.distributor_id, "REJECT")}
                          disabled={processingId === dist.distributor_id}
