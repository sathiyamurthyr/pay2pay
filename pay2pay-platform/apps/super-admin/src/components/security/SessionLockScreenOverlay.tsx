...
export const SessionLockScreenOverlay: React.FC = () => {
  ...
  // Dynamic Company & Super Admin/User Metadata from authenticated session
  const companyName = walletData?.company_name || "Pay2Pay Enterprise";
  const userName = walletData?.owner_name || walletData?.retailer_name || "Super Admin";
  const userCode = walletData?.retailer_code || (walletData as any)?.user_code || "SA-001";
  ...
  const handleGoToLogin = () => {
    try {
      const rawRole = localStorage.getItem("p2p_user_role") || "SUPER_ADMIN";
