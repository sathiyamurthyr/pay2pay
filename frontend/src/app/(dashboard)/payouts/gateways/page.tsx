...
export default function BankGatewaysPage() {
  ...
  const fetchLiveBalances = async () => {
    try {
      ...
      if (res.data?.status === "SUCCESS" && res.data?.data) {
        const gwBalances = res.data.data;
        setBalances((prev) => ({
          ...prev,
          ...gwBalances,
        }));
      } else {
        await refreshSingleBalance("WOWPE");
      }
    } catch (err) {
      await refreshSingleBalance("WOWPE");
    } finally {
  ...
  const handleSwitchPrimary = async (targetProvider: string) => {
    if (targetProvider === activePrimary || switching) return;

    try {
      setSwitching(true);
      try {
        await api.post("/api/v1/admin/payout-routing/switch", {
          provider_code: targetProvider,
          reason: `Admin manual switch to ${targetProvider}`,
        });
      } catch (e) {
        // Optimistic UI fallback
      }

      setActivePrimary(targetProvider);
      setGateways((prev) =>
        prev.map((g) => ({
          ...g,
          is_primary: g.provider_code === targetProvider,
          priority_order: g.provider_code === targetProvider ? 1 : 2,
        }))
      );
      setNotification({
        type: "success",
        message: `Primary payout engine switched to ${targetProvider}!`,
      });
    } finally {
      setSwitching(false);
  ...
  const handleTestConnection = async (providerCode: string) => {
    try {
      ...
      // Call backend test-connection with IPv4 routing
      try {
        ...
        if (res.data?.status === "SUCCESS") {
          ...
          setTestResult({
            ...
            success: true,
            message: `Live Ping 200 OK (${resp.latency_ms || 120}ms) • Server Message: "${resp.message || "Connected"}" • Live Balance: ₹${resp.balance || 0}`,
            details: resp,
          });
          return;
        }
                ...
                return (
                  <tr key={g.provider_code} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4 font-mono text-[11px] text-slate-500">
                      <div>User: <span className="text-slate-800 font-semibold">{g.user_id}</span></div>
                      <div>Secret: <span className="text-slate-800 font-semibold">{g.masked_secret || "••••••••••••"}</span></div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        <span className={`font-mono font-black text-sm ${g.provider_code === "WOWPE" ? "text-emerald-600" : "text-sky-600"}`}>
                          ₹{Number(gwBal?.balance ?? g.last_known_balance ?? 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                        </span>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase font-mono mb-1">User ID / Client ID</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase font-mono mb-1">Merchant / Node ID</label>
                  <input
                    type="text"
