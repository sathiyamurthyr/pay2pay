...
# Use the same password hasher as the app
...
# Get the schema of auth_users
cur.execute("PRAGMA table_info(auth_users);")
cols = [c[1] for c in cur.fetchall()]
print("auth_users columns:", cols)
...
# Get a tenant_id
cur.execute("SELECT id, public_id FROM tenants LIMIT 1;")
tenant = cur.fetchone()
if tenant:
    tenant_id = tenant[1]
    ...
    print(f"No tenant found, using: {tenant_id}")
...
for u in USERS:
    ...
    # Insert minimal required fields based on schema
    try:
        cur.execute("""
        ...
        """, (uid, pub_id, tenant_id, user_id, u["mobile_number"], u["full_name"], u["email"],
              pwd_hash, u["role"], "ACTIVE", 1, 0, 0, now, now))
